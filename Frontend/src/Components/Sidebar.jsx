...
const Sidebar = ({ onClose }) => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full w-72 bg-[#0a1627e6] backdrop-blur-md shadow-2xl p-6 z-50 text-white transform transition-transform duration-300 ${
      ...
      <nav className="flex flex-col gap-4">
        {navLinks.map((link, idx) => (
          <Link
            ...
            to={link.to}
            onClick={handleClose}
            className="flex items-center gap-4 text-base px-4 py-3 rounded-xl hover:bg-white/10 hover:shadow-inner transition duration-300"
          >
